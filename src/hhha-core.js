...
(function (global) {
  ...
  function bootstrapDatos() {
    ...
    if (state) {
      ...
      if (cambios > 0) {
        save({ internal: true });
        UI.notify(`Migración: ${cambios} inconsistencia${cambios > 1 ? 's' : ''} de versiones previas corregida${cambios > 1 ? 's' : ''}.`, 'success');
      }
    }
    if (!state) {
      state = init();
      // Reconstruir ciclos desde eventos seed (Solicitud de trabajo con folio)
      state.eventos.forEach(ev => {
        if (ev.tipo === 'Solicitud de trabajo' && ev.folio) {
      ...
      // Cerrar ciclos cuyo evento de cierre dejó operativo
      state.eventos.forEach(ev => {
      ...
      // Recalcular estado de cada equipo
  ...
  const HHHA = {
    ...
    normalizarEquipos, normalizarEstadoPend, limpiarEfectosAnulados, bootstrapDatos,
    // utilidades
