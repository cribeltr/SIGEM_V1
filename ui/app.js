...
(function () {
  ...
  function fromHash() {
    ...
    const v = m[0] || 'inicio';
    // 'equipo' (ficha), 'ciclos' (en Eventos) y 'asignaciones' (detalle MP, en la ficha + drill-down de Cumplimiento)
    // son vistas válidas aunque no estén en el menú.
    if (NAV.find(n => n.id === v) || v === 'equipo' || v === 'ciclos' || v === 'asignaciones') { view = v; params = m[1] ? { inv: decodeURIComponent(m[1]) } : {}; }
  }
  ...
  VIEWS.tablero = function () {
    ...
    const seg = h('div', { class: 'seg' }, ...[['estado', 'Por estado'], ['pendientes', 'Pendientes'], ['correctivos', 'Correctivos']].map(([v, l]) =>
      h('button', { class: modo === v ? 'on' : '', onclick: e => { if (modo === v) return; modo = v; [...seg.children].forEach(b => b.classList.remove('on')); e.target.classList.add('on'); paint(); } }, l)));
    paint();
  ...
  function boardCorrectivos() {
    const S = H.getState();
    const FASES = [['Solicitud de trabajo', 'Solicitud'], ['Visita técnica', 'Visita'], ['Orden de Compra', 'O. Compra'], ['Envío a servicio técnico', 'Envío'], ['Recepción', 'Recepción'], ['Reparación', 'Reparación']];
    const board = h('div', { class: 'kb-board' });
    const abiertos = S.ciclos.filter(c => c.estado === 'abierto');
    const faseActual = c => {
      const evs = S.eventos.filter(e => !e.anulado && FASES.some(f => f[0] === e.tipo) && (c.folio ? e.folio === c.folio : e.inv === c.inv));
      if (!evs.length) return 'Solicitud de trabajo';
      evs.sort((a, b) => (a.fecha || '').localeCompare(b.fecha || '') || (a.id - b.id));
      return evs[evs.length - 1].tipo;
    };
    const card = c => { const eq = H.findEquipo(c.inv) || {}; return kbCard([
      h('div', { class: 'kb-inv' }, c.folio || ('#' + c.id)),
    ...
    ], c.folio || ('#' + c.id), () => go('equipo', { inv: c.inv, tab: 'ciclos' }), 'var(--st)'); };
    function render() {
      board.innerHTML = '';
      const byFase = {}; FASES.forEach(f => byFase[f[0]] = []);
      abiertos.forEach(c => { const f = faseActual(c); (byFase[f] || byFase['Solicitud de trabajo']).push(c); });
      FASES.forEach(([tipo, label]) => board.appendChild(kbCol(label, 'var(--st)', byFase[tipo].length, byFase[tipo].map(card), id => {
    ...
    render();
    return h('div', {},
      h('div', { class: 'filterbar' }, h('span', { class: 'faint', style: { fontSize: '11px' } }, `${abiertos.length} ciclo(s) correctivo(s) abierto(s) · arrastra una tarjeta a una etapa para registrar ese evento`)),
      board);
  }
  function pendCard(p) {
  ...
  function eventoMini(e) {
    ...
    return h('div', { class: 'mini-row' + (e.anulado ? ' done' : ''), style: { display: 'block', cursor: e.anulado ? 'default' : 'pointer' }, onclick: e.anulado ? null : () => formEditarEvento(e) },
      ...
      h('div', { class: 'faint', style: { fontSize: '11px', marginTop: '2px' } }, [e.ejecutor, e.folio && ('Folio ' + e.folio)].filter(Boolean).join(' · ') || '—'),
      (() => { const x = []; if (e.nCotiz) x.push('Cotización ' + e.nCotiz); if (e.nOC) x.push('OC ' + e.nOC); if (e.empresa) x.push(e.empresa); if (e.tecnico) x.push('Téc. ' + e.tecnico); if (e.nEnvio) x.push('Envío ' + e.nEnvio); return x.length ? h('div', { class: 'faint', style: { fontSize: '11px', marginTop: '2px', color: 'var(--accent)' } }, x.join(' · ')) : null; })(),
  ...
  function cuadernoSheets() {
    ...
    // Correctivos: eventos no-MP con su detalle (N° informe, cotización, OC, empresa…), para consultar rápido.
    const TIPOS_CORR = ['Solicitud de trabajo', 'Visita técnica', 'Orden de Compra', 'Envío a servicio técnico', 'Recepción', 'Reparación'];
    sheets.push({
      name: 'Correctivos', hidden: false, rows: [
        ['Fecha', 'N° Inv.', 'Equipo', 'Servicio', 'Tipo', 'Estado equipo', 'N° Informe / Folio', 'N° Cotización', 'N° OC', 'Empresa', 'Técnico', 'N° Envío', 'Ejecutor', 'Observación'],
        ...S.eventos.filter(e => !e.anulado && TIPOS_CORR.includes(e.tipo)).sort((a, b) => (b.fecha || '').localeCompare(a.fecha || '')).map(e => [fF(e.fecha), e.inv, e.equipo || '', e.servicio || '', H.etiquetaTipoEvento(e), e.estado || '', e.folio || '', e.nCotiz || '', e.nOC || '', e.empresa || '', e.tecnico || '', e.nEnvio || '', e.ejecutor || '', e.obs || ''])
      ]
    });
    ...
    const mpRows = MESES.map((mm, i) => {
      const asig = (S.asignacionesMP || {})[`${YEAR}-${String(i + 1).padStart(2, '0')}`] || {};
      let prog = 0, asg = 0, ofi = 0, bor = 0, rep = 0, fs = 0, nu = 0, baja = 0, sin = 0;
      vivosMP.forEach(e => {
        const c = H.claseMPMes(e, YEAR, i); if (!c) return; prog++; if (asig[e.inv]) asg++;
