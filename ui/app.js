...
(function () {
  ...
  VIEWS.equipos = function () {
    const S = H.getState();
    let f = { q: params.q || '', estado: params.estado || 'todos', servicio: params.servicio || '', fam: params.fam || '', sinEnc: false };
    ...
    function render() {
      ...
      mount(tblWrap, h('table', { class: 'dense' },
        h('thead', {}, h('tr', {},
          th('inv', 'N° Inv.'), th('equipo', 'Equipo'), th('servicio', 'Servicio'),
          th('estado', 'Estado'), th('fam', 'Familia'), th('freq', 'Freq'), th('mp', `MP ${MES_ESP(MONTH)}`), h('th', {}, 'Encargado'), th('pend', 'Pend.', 'num'))),
        h('tbody', {}, ...list.map(e => h('tr', { onclick: () => go('equipo', { inv: e.inv }) },
  ...
  VIEWS.pendientes = function () {
    const S = H.getState();
    let f = { estado: params.vencidos ? 'activos' : (params.estado || 'activos'), tipo: '', q: '', serv: '', ejec: params.ejec || '' };
    pendSel = new Set();
  ...
  function pendientesTable(list, opts) {
    ...
    return h('div', { class: 'tbl-wrap' }, h('table', { class: 'dense' },
      h('thead', {}, h('tr', {}, headChk, h('th', {}, 'N° Inv.'), h('th', {}, 'Equipo'), h('th', {}, 'Tipo'), h('th', {}, 'Descripción'), h('th', {}, 'Estado'), h('th', {}, 'Responsable'), h('th', {}, 'Compromiso'))),
      h('tbody', {}, ...list.map(p => {
