...
(function () {
  ...
  function tabMatriz(eq) {
    ...
    const obsList = H.getState().eventos.filter(e => e.inv === eq.inv && !e.anulado && e.tipo === 'Mantención preventiva' && (e.obs || '').trim() && e.fecha && e.fecha.startsWith(String(yr))).sort((a, b) => (a.fecha || '').localeCompare(b.fecha || ''));
    return h('div', {},
      h('div', { class: 'section' },
        ...
        h('div', { class: 's-bd' }, h('div', { class: 'faint', style: { fontSize: '11.5px' } }, 'Click en una celda de Resultado para registrar/editar la MP de ese mes. La ✎ indica que esa MP tiene observación. Códigos: Si=hecha · C1–C8=reprogramación · FS/NU/No=falla · Baja.'))),
      h('div', { class: 'section' },
  ...
  function tabBitacora(eq) {
    const all = H.eventosDeTodos(eq.inv).reverse();
    let verAuto = false;
    const body = h('div', { class: 's-bd flush' });
    ...
    function render() {
      ...
      mount(body, evs.length ? eventosTable(evs, true) : h('div', { class: 'empty' }, verAuto ? 'Sin eventos' : 'Sin registros manuales'));
    }
  ...
  function tabPendientes(eq) {
    ...
    let filtro = 'activos';   // activos | resueltos | todos
    const body = h('div', { class: 's-bd flush' });
    const sub = h('span', { class: 's-sub' });
    function lista() {
      if (filtro === 'resueltos') return all.filter(p => p.estado === 'cerrado');
      if (filtro === 'todos') return all;
      return all.filter(p => p.estado !== 'cerrado');
    }
    function render() {
      const ps = lista();
      sub.textContent = `${nAct} activo(s) · ${nRes} resuelto(s)`;
      mount(body, ps.length ? h('div', { class: 'p-list' }, ...ps.map(pendienteCard))
        : h('div', { class: 'empty' }, filtro === 'resueltos' ? 'Sin pendientes resueltos' : filtro === 'activos' ? 'Sin pendientes activos' : 'Sin pendientes'));
    }
    const seg = h('div', { class: 'seg' }, ...[['activos', `Activos${nAct ? ' · ' + nAct : ''}`], ['resueltos', `Resueltos${nRes ? ' · ' + nRes : ''}`], ['todos', 'Todos']].map(([v, l]) =>
      h('button', { class: filtro === v ? 'on' : '', onclick: e => { filtro = v;[...seg.children].forEach(b => b.classList.remove('on')); e.target.classList.add('on'); render(); } }, l)));
    render();
    return h('div', { class: 'section' }, h('div', { class: 's-hd' }, h('h3', {}, 'Pendientes'), sub, h('div', { class: 'tb-spacer' }), seg,
      h('button', { class: 'btn sm primary', onclick: () => formNuevoPendiente({ inv: eq.inv }) }, svg(ic.plus, 14), 'Nuevo')), body);
  }
    ...
    function render() {
      ...
      mount(wrap, (list.length || cf.anyActive()) ? h('table', { class: 'dense' },
        h('thead', {}, h('tr', {}, h('th', { class: 'shrink' }, allChk),
          ...
          cf.thF('resp', 'Responsable', e => { const a = (S.asignacionesMP || {})[keyMes()] || {}; return a[e.inv] || 'sin asignar'; }), h('th', { class: 'shrink' }, ''))),
        h('tbody', {}, ...list.map(e => {
  ...
  VIEWS.cumplimiento = function () {
    ...
    function filasPorMes() {
      ...
      return MESES.map((mm, i) => {
        ...
        let prog = 0, asignadas = 0, realiz = 0, reprog = 0, fs = 0, nu = 0, baja = 0, sinReg = 0;
        vivos.forEach(e => {
          if (!H.mpProgramadaEnMes(e, mm)) return;
          prog++;
          if (asig[e.inv]) asignadas++;
          const r = H.resultadoMPMes(e, y, i);
          if (r === 'Si') realiz++;
          else if (/^C[1-8]$/.test(r || '')) reprog++;
          else if (r === 'FS') fs++;
          else if (r === 'NU') nu++;
          else if (r === 'Baja') baja++;
          else sinReg++;
        });
        return { mm, i, prog, asignadas, sinAsig: prog - asignadas, realiz, reprog, fs, nu, baja, sinReg, pct: prog ? Math.round(realiz / prog * 100) : null };
      });
    }
    ...
    function renderMes() {
      ...
      const T = rows.reduce((a, r) => ({ prog: a.prog + r.prog, asignadas: a.asignadas + r.asignadas, sinAsig: a.sinAsig + r.sinAsig, realiz: a.realiz + r.realiz, reprog: a.reprog + r.reprog, fs: a.fs + r.fs, nu: a.nu + r.nu, baja: a.baja + r.baja, sinReg: a.sinReg + r.sinReg }), { prog: 0, asignadas: 0, sinAsig: 0, realiz: 0, reprog: 0, fs: 0, nu: 0, baja: 0, sinReg: 0 });
      const Tpct = T.prog ? Math.round(T.realiz / T.prog * 100) : null;
      const num = (v, color) => h('td', { class: 'num', style: (color && v) ? { color } : null }, v || '—');
      ...
      const t1 = h('div', { class: 'section', style: { marginBottom: '12px' } },
        h('div', { class: 's-hd' }, h('h3', {}, `MP ${y} · por mes y resultado`), h('span', { class: 's-sub' }, 'Clic en un mes para abrir su programación')),
        h('div', { class: 's-bd flush' }, h('div', { class: 'tbl-wrap' }, h('table', { class: 'dense' },
          h('thead', {}, h('tr', {}, h('th', {}, 'Mes'), h('th', { class: 'num' }, 'Programadas'), h('th', { class: 'num' }, 'Realizadas'), h('th', { class: 'num' }, 'Reprog.'), h('th', { class: 'num' }, 'FS'), h('th', { class: 'num' }, 'NU'), h('th', { class: 'num' }, 'Baja'), h('th', { class: 'num' }, 'Sin registro'))),
          h('tbody', {},
            ...rows.map(r => h('tr', {}, mesCell(r), h('td', { class: 'num' }, r.prog || '—'), num(r.realiz, 'var(--op)'), num(r.reprog, 'var(--st)'), num(r.fs, 'var(--noop)'), num(r.nu, 'var(--noop)'), num(r.baja, 'var(--noop)'), h('td', { class: 'num muted' }, r.sinReg || '—'))),
            totRow(h('td', { class: 'num' }, T.prog), h('td', { class: 'num' }, T.realiz), h('td', { class: 'num' }, T.reprog), h('td', { class: 'num' }, T.fs), h('td', { class: 'num' }, T.nu), h('td', { class: 'num' }, T.baja), h('td', { class: 'num' }, T.sinReg)))))));
      const t2 = h('div', { class: 'section' },
        h('div', { class: 's-hd' }, h('h3', {}, `MP ${y} · cumplimiento por mes`), h('span', { class: 's-sub' }, 'Asignación y ejecución')),
        h('div', { class: 's-bd flush' }, h('div', { class: 'tbl-wrap' }, h('table', { class: 'dense' },
          h('thead', {}, h('tr', {}, h('th', {}, 'Mes'), h('th', { class: 'num' }, 'Programadas'), h('th', { class: 'num' }, 'Asignadas'), h('th', { class: 'num' }, 'Sin asignar'), h('th', { class: 'num' }, 'Ejecutadas'), h('th', { class: 'num' }, 'Pendientes'), h('th', {}, 'Cumplimiento'))),
          h('tbody', {},
            ...rows.map(r => h('tr', {}, mesCell(r), h('td', { class: 'num' }, r.prog || '—'), num(r.asignadas), h('td', { class: 'num', style: r.sinAsig ? { color: 'var(--st)' } : null }, r.sinAsig || '—'), num(r.realiz, 'var(--op)'), h('td', { class: 'num', style: r.sinReg ? { color: 'var(--noop)' } : null }, r.sinReg || '—'),
              h('td', { style: { minWidth: '150px' } }, h('div', { style: { display: 'flex', alignItems: 'center', gap: '7px' } }, bar(r.pct), pctPill(r.pct))))),
            totRow(h('td', { class: 'num' }, T.prog), h('td', { class: 'num' }, T.asignadas), h('td', { class: 'num' }, T.sinAsig), h('td', { class: 'num' }, T.realiz), h('td', { class: 'num' }, T.sinReg),
              h('td', {}, h('div', { style: { display: 'flex', alignItems: 'center', gap: '7px' } }, bar(Tpct), pctPill(Tpct)))))))));
    ...
    function exportar() {
      if (modo === 'mes') {
        const rows = filasPorMes().map(r => [MES_ESP(r.i), r.prog, r.realiz, r.reprog, r.fs, r.nu, r.baja, r.sinReg, r.asignadas, r.sinAsig, r.pct == null ? '' : r.pct + '%']);
        return exportTablaExcel('MP por mes', `MP por mes · ${y}`, ['Mes', 'Programadas', 'Realizadas', 'Reprog.', 'FS', 'NU', 'Baja', 'Sin registro', 'Asignadas', 'Sin asignar', 'Cumplimiento'], rows, `SIGEM_mp_por_mes_${y}.xlsx`);
      }
      if (modo === 'responsable') {
  ...
  function openCmdk() {
    ...
    const actions = [
      ...
      ['Ir: Ciclos', () => go('ciclos'), '↻'], ['Ir: Eventos', () => go('eventos'), '≡'], ['Ir: Asignaciones MP', () => go('asignaciones'), '▤'], ['Ir: Configuración', () => go('configuracion'), '⚙'],
  ...
  function renderView() {
    ...
    const node = v();
    mount($('#view'), node);
    const titles = { inicio: 'Cola de trabajo', equipos: 'Equipos', equipo: 'Ficha de equipo', pendientes: 'Pendientes', ciclos: 'Ciclos correctivos', eventos: 'Bitácora de eventos', asignaciones: 'Asignaciones MP', cumplimiento: 'Cumplimiento por servicio', configuracion: 'Configuración' };
