...
(function () {
  ...
  VIEWS.equipos = function () {
    ...
    if (params.caidos) chips.push('Equipos caídos');
    if (params.mpMes) chips.push('MP del mes pendientes');
    ...
    // Resumen del día: indicadores clicables que filtran la tabla o saltan a Pendientes
    // (traen a Equipos las señales que antes solo se veían en Inicio / Cumplimiento).
    const vivos = S.equipos.filter(e => e.estado !== 'baja');
    const nOper = vivos.filter(e => e.estado === 'operativo').length;
    const nCaidos = vivos.filter(e => e.estado === 'no_operativo' || e.estado === 'en_servicio_tecnico').length;
    const nMpMes = vivos.filter(e => H.mpProgramadaEnMes(e, MESES[MONTH]) && H.mpEstadoMes(e, YEAR, MONTH) === 'pendiente').length;
    const nSinGest = vivos.filter(e => (e.estado === 'no_operativo' || e.estado === 'en_servicio_tecnico') && H.diasEnEstado(e) > 30).length;
    const pendAb = S.pendientes.filter(p => !p.anulado && p.estado !== 'cerrado');
    const nVenc = pendAb.filter(p => p.fechaComp && p.fechaComp < H.hoyLocal()).length;
    const nSinResp = pendAb.filter(p => !p.ejecutor).length;
    const pctOper = vivos.length ? Math.round(nOper / vivos.length * 100) : 0;
    const kpi = (cls, n, label, onclick, active) => h('button', { class: 'eq-kpi ' + cls + (active ? ' active' : ''), onclick }, h('b', {}, n), h('span', {}, label));
    const resumen = h('div', { class: 'eq-resumen' },
      kpi('op', pctOper + '%', nOper + ' operativos', () => go('equipos', { estado: 'operativo' }), params.estado === 'operativo'),
      kpi('noop', nCaidos, 'caídos', () => go('equipos', { caidos: 1 }), !!params.caidos),
      kpi('st', nMpMes, 'MP del mes pend.', () => go('equipos', { mpMes: 1 }), !!params.mpMes),
      kpi('noop', nSinGest, 'sin gestión >30 d', () => go('equipos', { alerta30: 1 }), !!params.alerta30),
      h('span', { class: 'eq-kpi-sep' }),
      kpi('noop', nVenc, 'pendientes vencidos', () => go('pendientes', { vencidos: 1 })),
      kpi('st', nSinResp, 'pend. sin responsable', () => go('pendientes', { ejec: '__none' })));
  ...
  VIEWS.equipo = function () {
    ...
    const head = h('div', { class: 'eq-head' },
      ...
      h('div', { class: 'btn-row' },
        ...
        eq.estado !== 'baja' ? h('button', { class: 'btn sm', title: 'Registrar la mantención preventiva de este mes', onclick: () => formMP(eq.inv) }, svg(ic.check, 14), 'MP del mes') : null,
        ...
        H.ciclosAbiertosDe(eq.inv).length ? h('button', { class: 'btn sm', title: 'Cerrar el ciclo correctivo (el equipo vuelve a operativo)', onclick: () => formNuevoEvento({ inv: eq.inv, tipo: 'Reparación', estado: 'operativo' }) }, svg(ic.check, 14), 'Cerrar ciclo') : null,
        h('button', { class: 'btn sm', onclick: () => formNuevoPendiente({ inv: eq.inv }) }, svg(ic.plus, 14), 'Pendiente'),
  ...
  VIEWS.pendientes = function () {
    ...
    function base() {
      ...
      if (params.semana) { const lim = H.addDias(H.hoyLocal(), 7); list = list.filter(p => p.fechaComp && p.fechaComp >= H.hoyLocal() && p.fechaComp <= lim && p.estado !== 'cerrado'); }
    ...
    // Resumen del día: prioriza de un vistazo (clic = filtra).
    const activos = S.pendientes.filter(p => !p.anulado && p.estado !== 'cerrado');
    const hoyP = H.hoyLocal(); const lim7 = H.addDias(hoyP, 7);
    const nVenc = activos.filter(p => p.fechaComp && p.fechaComp < hoyP).length;
    const nSem = activos.filter(p => p.fechaComp && p.fechaComp >= hoyP && p.fechaComp <= lim7).length;
    const nSinR = activos.filter(p => !p.ejecutor).length;
    const kpiP = (cls, n, label, onclick, active) => h('button', { class: 'eq-kpi ' + cls + (active ? ' active' : ''), onclick }, h('b', {}, n), h('span', {}, label));
    const resumenP = h('div', { class: 'eq-resumen' },
      kpiP('', activos.length, 'activos', () => go('pendientes', {}), !params.vencidos && !params.semana && f.ejec !== '__none'),
      kpiP('noop', nVenc, 'vencidos', () => go('pendientes', { vencidos: 1 }), !!params.vencidos),
      kpiP('st', nSem, 'esta semana', () => go('pendientes', { semana: 1 }), !!params.semana),
      kpiP('st', nSinR, 'sin responsable', () => go('pendientes', { ejec: '__none' }), f.ejec === '__none'));
    const root = h('div', {},
      resumenP,
      h('div', { class: 'filterbar' },
  ...
  function pendientesTable(list, opts) {
    ...
    return h('div', { class: 'tbl-wrap' }, h('table', { class: 'dense' },
      ...
      h('tbody', {}, ...list.map(p => {
        ...
        const ult = ultActP(p);
        const tr = h('tr', { class: (sel && sel.has(p.id) ? 'sel ' : '') + (venc ? 'pend-venc' : ''), onclick: () => formPendiente(p) },
          chk,
