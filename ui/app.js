...
(function () {
  ...
  const APP_VERSION = '2026-06-03 · v3.10';   // sello de build visible (barra superior y Configuración) para confirmar despliegue
  ...
  let view = 'inicio', params = {}, fichaOrigen = 'equipos';
  ...
  function exportarEquipos(list) {
    const header = ['ID', 'N° Inv.', 'Equipo', 'Serie', 'Servicio', 'Unidad', 'Ubicación', 'Marca', 'Modelo', 'Estado', 'Freq MP', 'Tiempo en estado (días)', 'Última actualización', 'Encargado', 'Pend. abiertos'];
    const rows = list.map(e => { const g = H.ultimaGestion(e.inv); return [e.id, e.inv, e.equipo || '', e.serie || '', e.servicio || '', e.unidad || '', e.ubic || '', e.marca || '', e.modelo || '', ESTADO_LABEL[e.estado] || e.estado, e.freq || '', H.diasEnEstado(e), g ? fmtFecha(g.fecha) : '', H.encargadoDe(e) || '', H.pendientesDe(e.inv).filter(p => p.estado !== 'cerrado').length]; });
  ...
  function exportarEquiposMes(list) {
    const header = ['Fam', 'ID', 'N° Carpeta', 'N° Inventario', 'Equipo', 'Servicio', 'Unidad', 'Ubicación', 'Procedencia', 'Marca', 'Modelo', 'Serie', 'Año Instalación', 'Vida Útil Residual', 'Mes', 'Tipo', 'Resultado', 'Oficial', 'Ejecutor'];
    ...
    const rows = [];
    list.forEach(e => { if (e.estado === 'baja') return; for (let i = 0; i < 12; i++) { if (!H.mpProgramadaEnMes(e, MESES[i])) continue; rows.push([e.fam || '', e.id, e.carpeta || '', e.inv, e.equipo || '', e.servicio || '', e.unidad || '', e.ubic || '', e.proc || '', e.marca || '', e.modelo || '', e.serie || '', e.ano || '', e.vur || '', MES_ESP(i), (e.prog || {})[MESES[i]] || '', H.resultadoMPMes(e, YEAR, i) || 'Pendiente', oficMes(e, i), ejecMes(e, i)]); } });
  ...
  VIEWS.pendientes = function () {
    ...
    let vmode = 'cards'; const wrap = h('div', {}); const note = h('span', { class: 'count-note' });
